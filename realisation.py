...
class Realisation:
    def __init__(self,root):
        ...
        ## Title
        title_lbl=Label(self.root,text="Realisation Account",font=("times new roman",30,"bold"),bg="white",fg="darkgreen")
        ...
        left_frame=LabelFrame(main_frame,bd=2,bg='white',relief=RIDGE,text='Realisation Account Enteries Details',font=('times new roman',12,"bold"))
        left_frame.place(x=10,y=10,width=550,height=510)

        #### image for left frame
        img_left=Image.open(os.path.join("account_images","realisation_left.jpg"))
        ...
        # realisation enteries frame
        realisation_frame=LabelFrame(left_frame,bd=2,bg='white',relief=RIDGE,text='Realisationn Account Enteries',font=('times new roman',12,"bold"))
        realisation_frame.place(x=5,y=125,width=540,height=360)


        # Particulars Labels
        particular_label=Label(realisation_frame,text='Particulars:',font=('times new roman',13,"bold"),bg='white')
        particular_label.grid(row=0,column=0,padx=10,pady=15,sticky=W)

        particular_entry=ttk.Entry(realisation_frame,textvariable=self.var_particular1,width=30,font=('times new roman',13,"bold"))
        particular_entry.grid(row=0,column=1,padx=10,pady=15,sticky=W)


        # amount1 Labels
        amount1_label=Label(realisation_frame,text='Dr.Amount:',font=('times new roman',13,"bold"),bg='white')
        amount1_label.grid(row=1,column=0,padx=10,pady=15,sticky=W)

        amount1_entry=ttk.Entry(realisation_frame,textvariable=self.var_amount1,width=30,font=('times new roman',13,"bold"))
        ...
        particular_entry=ttk.Entry(realisation_frame,textvariable=self.var_particular2,width=30,font=('times new roman',13,"bold"))
        particular_entry.grid(row=2,column=1,padx=10,pady=15,sticky=W)

        # amount2 labels
        amount2_label=Label(realisation_frame,text='Cr.Amount:',font=('times new roman',13,"bold"),bg='white')
        amount2_label.grid(row=3,column=0,padx=10,pady=15,sticky=W)

        amount2_entry=ttk.Entry(realisation_frame,textvariable=self.var_amount2,width=30,font=('times new roman',13,"bold"))
        amount2_entry.grid(row=3,column=1,padx=10,pady=15,sticky=W)



        ### frames for left side button
        btn_frame=Frame(realisation_frame,bd=2,relief=RIDGE,bg='white')
...
################################################################################################
        ...
        right_frame=LabelFrame(main_frame,bd=2,bg='white',relief=RIDGE,text='realisation Enteries Details',font=('times new roman',12,"bold"))
        right_frame.place(x=580,y=10,width=730,height=510)

        ### image for right frame
        img_right=Image.open(os.path.join("account_images","realisation_right.jpg"))
        ...
        search_combo['values']=("select","particulars1","particulars2")
...
######################################### Table for data show ############################################
        ...
        self.realisation_table=ttk.Treeview(table_frame,column=("particulars1",'amount1','particulars2','amount2'),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)

        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=self.realisation_table.xview)
        scroll_y.config(command=self.realisation_table.yview)


        # for table header
        self.realisation_table.heading('particulars1',text="Particulars")
        self.realisation_table.heading('amount1',text="Dr.Amount")
        self.realisation_table.heading('particulars2',text="Particulars")
        self.realisation_table.heading('amount2',text="Cr.Amount")
        
        self.realisation_table['show']='headings'

        #setting with for column
        self.realisation_table.column('particulars1',width=230)
        self.realisation_table.column('amount1',width=120)
        self.realisation_table.column('particulars2',width=230)
        self.realisation_table.column('amount2',width=120)

        #packing table
        self.realisation_table.pack(fill=BOTH,expand=1)
...
### function for save data ###########
    def add_data(self):
        try:
            ...
            my_cursor.execute("insert into realisation values(%s,%s,%s,%s)",(
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Sucess","realisation details has been added Sucessfully",parent=self.root)
...
###  fetch data from database ###########
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="root",database="accounting_software")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from realisation")
        data=my_cursor.fetchall()

        if len(data)!=0:
            self.realisation_table.delete(*self.realisation_table.get_children())
            for i in data:
                self.realisation_table.insert('',END,values=i)
            conn.commit()
        conn.close()
    ...
    def search_data(self):
        if self.var_searchtxt.get()=="" or self.var_search.get()=="Select Option":
            ...
            try:
                conn=mysql.connector.connect(host='localhost',username='root',password="root",database="accounting_software")
                my_cursor=conn.cursor()
                query = f"SELECT * FROM realisation WHERE {self.var_search.get().lower()} LIKE '%{self.var_searchtxt.get().lower()}%'"

...
class Capital:
    def __init__(self,root):
        ...
        ## Title
        title_lbl=Label(self.root,text="Capital Account",font=("times new roman",30,"bold"),bg="white",fg="darkgreen")
        ...
        left_frame=LabelFrame(main_frame,bd=2,bg='white',relief=RIDGE,text='Capital Account Details',font=('times new roman',12,"bold"))
        left_frame.place(x=10,y=10,width=550,height=510)

        #### image for left frame
        img_left=Image.open(os.path.join("account_images","capital_left.jpg"))
        ...
        ### capital account  frame
        capital_frame=LabelFrame(left_frame,bd=2,bg='white',relief=RIDGE,text='Capital Account Enteries',font=('times new roman',12,"bold"))
        capital_frame.place(x=5,y=125,width=540,height=360)

############## frame and labels for debit side ######################
        debit_frame=LabelFrame(capital_frame,bd=2,bg='white',relief=RIDGE,text='Debit Side',font=('times new roman',12,"bold"))
...
# ############## frame and labels for credit side ##############
        credit_frame=LabelFrame(capital_frame,bd=2,bg='white',relief=RIDGE,text='Credit Side',font=('times new roman',12,"bold"))
        ...
        btn_frame=Frame(capital_frame,bd=2,relief=RIDGE,bg='white')
...
################################################################################################
        ...
        right_frame=LabelFrame(main_frame,bd=2,bg='white',relief=RIDGE,text='Capital Account enteries Details',font=('times new roman',12,"bold"))
        right_frame.place(x=580,y=10,width=730,height=510)

        ### image for right frame
        img_right=Image.open(os.path.join("account_images","capital_right.jpg"))
...
######################################### Table for data show ############################################
        ...
        self.capital_table=ttk.Treeview(table_frame,column=('date1',"particulars1",'partner1','partner2','date2',"particulars2",'partner3','partner4'),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)

        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)

        scroll_x.config(command=self.capital_table.xview)
        scroll_y.config(command=self.capital_table.yview)


        # for table header
        self.capital_table.heading('date1',text="Date")
        self.capital_table.heading('particulars1',text="Particulars")
        self.capital_table.heading('partner1',text="Partner A")
        self.capital_table.heading('partner2',text="Partner B")


        self.capital_table.heading('date2',text="Date")
        self.capital_table.heading('particulars2',text="Particulars")
        self.capital_table.heading('partner3',text="Partner A")
        self.capital_table.heading('partner4',text="Partner B")

        self.capital_table['show']='headings'

        #setting width for column
        self.capital_table.column('date1',width=100)
        self.capital_table.column('particulars1',width=250)
        self.capital_table.column('partner1',width=100)
        self.capital_table.column('partner2',width=200)

        self.capital_table.column('date2',width=100)
        self.capital_table.column('particulars2',width=250)
        self.capital_table.column('partner3',width=100)
        self.capital_table.column('partner4',width=200)

        #packing table
        self.capital_table.pack(fill=BOTH,expand=1)
...
### function for save data ###########
    def add_data(self):
        try:
                ...
                my_cursor.execute("insert into capital values(%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                self.var_date1.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Sucess","Capital Account details has been added Sucessfully",parent=self.root)
...
###  fetch data from database ###########
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="root",database="accounting_software")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from capital")
        data=my_cursor.fetchall()

        if len(data)!=0:
            self.capital_table.delete(*self.capital_table.get_children())
            for i in data:
                self.capital_table.insert('',END,values=i)
            conn.commit()
        conn.close()
    ...
    def search_data(self):
        if self.var_searchtxt.get()=="" or self.var_search.get()=="Select Option":
            ...
            try:
                conn=mysql.connector.connect(host='localhost',username='root',password="root",database="accounting_software")
                my_cursor=conn.cursor()
                query = f"SELECT * FROM capital WHERE {self.var_search.get().lower()} LIKE '%{self.var_searchtxt.get().lower()}%'"
